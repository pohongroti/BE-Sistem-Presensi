...
app.get('/admin/pengaturan/:idlokasi',(req, res)=>{
    if (!req.session.loggedin) return res.redirect('/');
    connectionKeKolam.query('SELECT * FROM pengaturan', (err, results) => {
       if(err) throw err
       const pengaturanDataDetail = results.filter(function(data) { return data.idlokasi === req.params.idlokasi});
...
app.post('/admin/addpengaturan', (req, res)=>{
    const idlokasi = req.body.idlokasi;
    const namalokasi = req.body.namalokasi;
    const longitude = req.body.longitude;
    const latitude = req.body.latitude;
    const jarakpresensi = req.body.jarakpresensi;
    connectionKeKolam.query(
        `INSERT INTO pengaturan (idlokasi,namalokasi,longitude,latitude,jarak_presensi) VALUES ("${idlokasi}","${namalokasi}","${longitude}","${latitude}","${jarakpresensi}")`, (err, results) =>{
            if (err) throw err;
            if(results.length=1){
                console.log('Input Data Lokasi berhasil dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            } else {
                console.log('Input Data Lokasi gagal dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            }
        }
    );
...
app.post('/admin/editpengaturan', (req, res)=>{
    ...
    const jarakpresensi = req.body.jarakpresensi;
    connectionKeKolam.query(
        `UPDATE pengaturan SET namalokasi="${namalokasi}",longitude="${longitude}",latitude="${latitude}",jarak_presensi="${jarakpresensi}" WHERE idlokasi="${idlokasi}"`, (err, results) =>{
            if (err) throw err;
            if(results.length=1){
                console.log('Edit Data Lokasi berhasil dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            } else {
                console.log('Edit Data Lokasi gagal dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            }
        }
    );
...
app.get('/admin/deletepengaturan/:idlokasi', (req, res)=>{
    if (!req.session.loggedin) return res.redirect('/');
    const idlokasi = req.params.idlokasi;
    connectionKeKolam.query(
        `DELETE FROM pengaturan WHERE idlokasi="${idlokasi}"`, (err, results) =>{
            if (err) throw err;
            if(results.length=1){
                console.log('Delete Data Lokasi berhasil dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            } else {
                console.log('Delete Data Lokasi gagal dengan idlokasi='+idlokasi);
                res.redirect('/admin/pengaturan');
            }
        }
    );
...
app.get('/admin/laporan',(req, res)=>{
    if (!req.session.loggedin) return res.redirect('/');
    connectionKeKolam.query('SELECT pegawai.idpegawai,pegawai.namapegawai,pegawai.jabatanpegawai,presensi.idpresensi,presensi.tglwaktu,presensi.longitude,presensi.latitude,presensi.jenis_presensi FROM presensi,pegawai WHERE presensi.idpegawai=pegawai.idpegawai', (err, results) => {
       if(err) throw err
